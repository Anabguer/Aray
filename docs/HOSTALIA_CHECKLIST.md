...
- `dist/` → raíz pública `/aray/` (o copiar `index.html` + `assets/` a `/aray/`)
- `api/`
...
- App: `https://intocables13.com/aray/`
- API: `https://intocables13.com/aray/api/v1/`
- El cliente usa `import.meta.env.BASE_URL + 'api/v1'` → `/aray/api/v1`
- Cookies: path `/aray` (`ARAYSESSID`, `ARAYDEVICE`)
...
- SPA → `index.html`
- Comprobar que `/aray/adult` no 404

## 7. Pruebas post-subida

| # | Prueba | Cómo |
|---|--------|------|
| 1 | Conexión BD | `GET /aray/api/v1/health.php` → `dbOk`, versiones |
| 2 | Login Neni | PIN adulto / adult-login |
| 3 | Autorizar dispositivo | Panel → «Autorizar este dispositivo» |
| 4 | Snapshot | `GET /aray/api/v1/players/progress.php?playerId=` → XP oficial |
...
| 8 | Panel / premios / curso | Siguen OK |
...
2. Restaurar árbol `/aray/` anterior por FTP.
...
1. Dump BD → 2. Subir código → 3. `database.local.php` → 4. health → 5. (opcional) reset SQL confirmado → 6. epoch client → 7. authorize device → 8. partida real → 9. checklist OK.
