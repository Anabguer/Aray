# Errores reales - C / Z / QU

## Objetivo

Este documento recoge errores ortográficos reales relacionados con **c**, **z** y **qu** para alumnos de 3.º de Primaria en Cataluña.

Este documento es la fuente oficial para crear posteriormente los bancos JSON.

Nunca debe contener palabras inventadas.

**Metodología:** misma que los bancos H, BV, GJ, RR y LL/Y — fichas del repositorio primero; no copiar ejercicios; sin conjugaciones de relleno.

**Propiedad de lemas (norma del proyecto):** cada palabra tiene un único banco propietario. No se duplican registros.

| Lema / grupo | Banco propietario |
|--------------|-------------------|
| haber, hacer, echar | `ERRORES_REALES_H.md` |
| hay / ahí / ay | `ERRORES_REALES_HAY_AHI_AY.md` |
| caballo, llave, botella… | `ERRORES_REALES_BV.md` |
| cigüeña / guitarra (gü/gu) | futuro GU/GÜ |

**Cruces (sin duplicar contenido):**
- Foco: **ca/co/cu** vs **que/qui**; **za/zo/zu** vs **ce/ci**; finales en **-z**.
- **cigüeña** → gü (fuera).
- **bicicleta** → fuera (sin drill lengua c/z; además b).
- **quinto** → fuera (sin drill ortográfico claro).
- Nombres propios y conjugaciones → fuera.

---

## Correcta

queso

## Error frecuente 1

ceso

## Error frecuente 2



## Regla

Ante e, i el sonido /k/ se escribe qu.

## Frecuencia

Muy frecuente

## Categoría

Comida

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo; Savia; Vicens; `formar-palabras.json`.

---

## Correcta

zapato

## Error frecuente 1

capato

## Error frecuente 2



## Regla

Ante a, o, u el sonido /θ/~/s/ se escribe z.

## Frecuencia

Muy frecuente

## Categoría

Objetos

## Imagen recomendable

Sí

## Observaciones

Vicens (_apato). Aray `zapato-ctx`; `formar-palabras.json`.

---

## Correcta

cocina

## Error frecuente 1

cozina

## Error frecuente 2



## Regla

Ante i el sonido /θ/~/s/ se escribe c (no z).

## Frecuencia

Muy frecuente

## Categoría

Casa

## Imagen recomendable

Sí

## Observaciones

Vicens (co_ina). `formar-palabras.json`.

---

## Correcta

zumo

## Error frecuente 1

cumo

## Error frecuente 2



## Regla

Ante u el sonido /θ/~/s/ se escribe z.

## Frecuencia

Muy frecuente

## Categoría

Comida

## Imagen recomendable

Sí

## Observaciones

Vicens (_umo).

---

## Correcta

cielo

## Error frecuente 1

zielo

## Error frecuente 2



## Regla

Ante e, i el sonido /θ/~/s/ se escribe c (no z).

## Frecuencia

Muy frecuente

## Categoría

Naturaleza

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (_ielo).

---

## Correcta

casa

## Error frecuente 1

quasa

## Error frecuente 2



## Regla

Ante a, o, u el sonido /k/ se escribe c (no qu).

## Frecuencia

Muy frecuente

## Categoría

Casa

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (_asa); Savia.

---

## Correcta

lápiz

## Error frecuente 1

lápis

## Error frecuente 2



## Regla

Algunas palabras terminan en z.

## Frecuencia

Muy frecuente

## Categoría

Colegio

## Imagen recomendable

Sí

## Observaciones

Aray `lapiz-ctx`; Vicens; Savia; `formar-palabras.json`.

---

## Correcta

ciudad

## Error frecuente 1

ziudad

## Error frecuente 2



## Regla

Ante e, i el sonido /θ/~/s/ se escribe c (no z).

## Frecuencia

Muy frecuente

## Categoría

Ciudad

## Imagen recomendable

Sí

## Observaciones

Aray `ciudad-ctx`.

---

## Correcta

cocodrilo

## Error frecuente 1

cozodrilo

## Error frecuente 2



## Regla

Ante o el sonido /k/ se escribe c.

## Frecuencia

Muy frecuente

## Categoría

Animales

## Imagen recomendable

Sí

## Observaciones

Savia u3 (cocodrilos).

---

## Correcta

cebolla

## Error frecuente 1

zebolla

## Error frecuente 2



## Regla

Ante e el sonido /θ/~/s/ se escribe c (no z).

## Frecuencia

Muy frecuente

## Categoría

Comida

## Imagen recomendable

Sí

## Observaciones

Vicens (_ebolla).

---

## Correcta

pequeño

## Error frecuente 1

peceño

## Error frecuente 2



## Regla

Ante e, i el sonido /k/ se escribe qu.

## Frecuencia

Frecuente

## Categoría

Otros

## Imagen recomendable

No

## Observaciones

ANAYA refuerzo (pe_ueño).

---

## Correcta

macarrones

## Error frecuente 1

maquarrones

## Error frecuente 2



## Regla

Ante a el sonido /k/ se escribe c (no qu).

## Frecuencia

Frecuente

## Categoría

Comida

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (ma_arrones).

---

## Correcta

azul

## Error frecuente 1

acul

## Error frecuente 2



## Regla

Ante u el sonido /θ/~/s/ se escribe z.

## Frecuencia

Frecuente

## Categoría

Otros

## Imagen recomendable

Sí

## Observaciones

Vicens (a_ul).

---

## Correcta

cisne

## Error frecuente 1

zisne

## Error frecuente 2



## Regla

Ante i el sonido /θ/~/s/ se escribe c (no z).

## Frecuencia

Frecuente

## Categoría

Animales

## Imagen recomendable

Sí

## Observaciones

Vicens (_isne).

---

## Correcta

cubo

## Error frecuente 1

qubo

## Error frecuente 2



## Regla

Ante u el sonido /k/ se escribe c (no qu).

## Frecuencia

Frecuente

## Categoría

Objetos

## Imagen recomendable

Sí

## Observaciones

Vicens (_ubo).

---

## Correcta

azúcar

## Error frecuente 1

acúcar

## Error frecuente 2



## Regla

Ante u el sonido /θ/~/s/ se escribe z.

## Frecuencia

Frecuente

## Categoría

Comida

## Imagen recomendable

Sí

## Observaciones

Vicens (a_úcar).

---

## Correcta

cazo

## Error frecuente 1

caso

## Error frecuente 2



## Regla

Ante o el sonido /θ/~/s/ se escribe z.

## Frecuencia

Frecuente

## Categoría

Casa

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (ca_o). Caso es otra palabra.

---

## Correcta

maceta

## Error frecuente 1

mazeta

## Error frecuente 2



## Regla

Ante e el sonido /θ/~/s/ se escribe c (no z).

## Frecuencia

Frecuente

## Categoría

Casa

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (ma_eta).

---

## Correcta

receta

## Error frecuente 1

rezeta

## Error frecuente 2



## Regla

Ante e el sonido /θ/~/s/ se escribe c (no z).

## Frecuencia

Frecuente

## Categoría

Comida

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (re_eta).

---

## Correcta

ciervo

## Error frecuente 1

ziervo

## Error frecuente 2



## Regla

Ante i el sonido /θ/~/s/ se escribe c (no z).

## Frecuencia

Frecuente

## Categoría

Animales

## Imagen recomendable

Sí

## Observaciones

ANAYA ampliación (_ier_o).

---

## Correcta

cazuela

## Error frecuente 1

cacuela

## Error frecuente 2



## Regla

Ante a, o, u el sonido /θ/~/s/ se escribe z.

## Frecuencia

Frecuente

## Categoría

Casa

## Imagen recomendable

Sí

## Observaciones

ANAYA ampliación (ca_uela).

---

## Correcta

paquete

## Error frecuente 1

pacete

## Error frecuente 2



## Regla

Ante e, i el sonido /k/ se escribe qu.

## Frecuencia

Frecuente

## Categoría

Objetos

## Imagen recomendable

Sí

## Observaciones

Savia u3; Vicens.

---

## Correcta

esquina

## Error frecuente 1

escina

## Error frecuente 2



## Regla

Ante e, i el sonido /k/ se escribe qu.

## Frecuencia

Frecuente

## Categoría

Ciudad

## Imagen recomendable

Sí

## Observaciones

Savia u3; Vicens (es_quina).

---

## Correcta

cereza

## Error frecuente 1

zereza

## Error frecuente 2



## Regla

Ante e el sonido /θ/~/s/ se escribe c (no z).

## Frecuencia

Frecuente

## Categoría

Comida

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (cerezas).

---

## Correcta

corazón

## Error frecuente 1

corasón

## Error frecuente 2



## Regla

Algunas palabras se escriben con z.

## Frecuencia

Frecuente

## Categoría

Cuerpo

## Imagen recomendable

Sí

## Observaciones

`formar-palabras.json`; Naturales 3.º. Foco z (no b/v).

---

## Correcta

culebra

## Error frecuente 1

qulebra

## Error frecuente 2



## Regla

Ante u el sonido /k/ se escribe c (no qu).

## Frecuencia

Frecuente

## Categoría

Animales

## Imagen recomendable

Sí

## Observaciones

Savia u3 (la culebra).

---

## Correcta

quince

## Error frecuente 1

cince

## Error frecuente 2



## Regla

Ante e, i el sonido /k/ se escribe qu.

## Frecuencia

Frecuente

## Categoría

Otros

## Imagen recomendable

No

## Observaciones

Savia (quince minutos). Quincena aparece en ANAYA amp.; se prefiere el lema *quince*.

---

## Correcta

cumpleaños

## Error frecuente 1

qumpleaños

## Error frecuente 2



## Regla

Ante a el sonido /k/ se escribe c (no qu).

## Frecuencia

Frecuente

## Categoría

Otros

## Imagen recomendable

Sí

## Observaciones

Savia u3 (cumpleaños).

---

## Correcta

cebra

## Error frecuente 1

zebra

## Error frecuente 2



## Regla

Ante e el sonido /θ/~/s/ se escribe c (no z).

## Frecuencia

Poco frecuente

## Categoría

Animales

## Imagen recomendable

Sí

## Observaciones

ANAYA ampliación (animal con rayas → cebra).

---

## Correcta

erizo

## Error frecuente 1

erico

## Error frecuente 2



## Regla

Algunas palabras se escriben con z.

## Frecuencia

Poco frecuente

## Categoría

Animales

## Imagen recomendable

Sí

## Observaciones

ANAYA ampliación (mamífero con púas → erizo).

---

## Correcta

caramelo

## Error frecuente 1

quaramelo

## Error frecuente 2



## Regla

Ante a el sonido /k/ se escribe c (no qu).

## Frecuencia

Poco frecuente

## Categoría

Comida

## Imagen recomendable

Sí

## Observaciones

Savia u3 (caramelos).

---

## Resumen editorial

- **Número de registros (borrador 1):** 30
- **Fichas / PDFs revisados:**
  - `Lengua 3º-ANAYA refuerzo.pdf` p.5 (c/qu y c/z)
  - `Lengua 3º-ANAYA ampliacion.pdf` p.5 (c/z/qu + animales)
  - `refuerzo_ampliacion_lengua_3.pdf` (Vicens: c/z + ca/que/qui)
  - `lengua_3_savia.pdf` u3 (ca/co/cu/que/qui)
  - Aray `zapato-ctx`, `ciudad-ctx`, `lapiz-ctx`
  - `formar-palabras.json` (queso, cocina, zapato, lápiz, corazón)
  - `docs/TEMARIO_3_PRIMARIA_CATALUNYA.md`
- **No incluidos:** bicicleta, quinto, ceniza, cepillo; cigüeña (gü); nombres; conjugaciones; cola ANAYA dudosa (cerveza, cizaña, zurcir…).
- **Criterios:** solo c/z/qu; lemas; sin inventar.

### Dudas editoriales

- **casa / cumpleaños / culebra / caramelo:** ¿errores (*quasa*, *qumpleaños*, *qulebra*, *quaramelo*) suficientemente naturales?
- **cazo / caso:** homófonos; ¿mantener con nota?
- **corazón:** ¿OK en CZ/QU o demasiado “Naturales”?
- **quince vs quincena:** ¿lema correcto?
- **cebra / erizo / caramelo:** cola; ¿recortar?
- **ceniza, cepillo, quinto, cocodrilo (ya incluido), bicicleta:** brief; fuera salvo cocodrilo.
- **OCR ANAYA amp.** (cerveza, zarzamora…): no forzados; confirmar en auditoría si se recuperan.
- **Categoría Otros** (pequeño, azul, quince, cumpleaños): reasignar si cabe.

### Autoauditoría (borrador 1)

- Sin duplicados.
- Sin bicicleta / cigüeña / nombres.
- Errores orientados a c↔z↔qu.
- Vocabulario 3.º respaldado.
- Frecuencias: Muy frecuente 10 · Frecuente 17 · Poco frecuente 3.
- Pendiente de congelar tras auditoría humana.
