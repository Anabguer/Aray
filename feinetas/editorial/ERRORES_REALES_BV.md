# Errores reales - B / V

## Objetivo

Este documento recoge errores ortográficos reales relacionados con **b** y **v** para alumnos de 3.º de Primaria en Cataluña.

Este documento es la fuente oficial para crear posteriormente los bancos JSON.

Nunca debe contener palabras inventadas.

**Metodología:** misma que `ERRORES_REALES_H.md` — fichas del repositorio primero; no copiar ejercicios; sin conjugaciones de relleno.

**Cruces:**
- Combinaciones **mb / mp / nv** → documento `ERRORES_REALES_MPMB.md` (no duplicar aquí).
- Terminaciones **-aba** (cantaba…) → se tratan como regla propia en el temario; no se usan como relleno de conjugaciones en este banco.
- Palabras cuya dificultad principal es **h** (haber, hacer…) → si aparecen, el error aquí es solo el de **b/v** (p. ej. *haver*), no la omisión de h.

---

## Correcta

caballo

## Error frecuente 1

cavallo

## Error frecuente 2



## Regla

Caballo se escribe con b.

## Frecuencia

Muy frecuente

## Categoría

Animales

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo/ampliación; Vicens (caballos); contexto Aray caballo-ctx.

---

## Correcta

vaca

## Error frecuente 1

baca

## Error frecuente 2



## Regla

Vaca se escribe con v. (Baca es otra palabra.)

## Frecuencia

Muy frecuente

## Categoría

Animales

## Imagen recomendable

Sí

## Observaciones

ANAYA ampliación («La vaca come…»). Homófonos reales vaca/baca.

---

## Correcta

burro

## Error frecuente 1

vurro

## Error frecuente 2



## Regla

Burro se escribe con b.

## Frecuencia

Frecuente

## Categoría

Animales

## Imagen recomendable

Sí

## Observaciones

Fábula del material 3.º (niño, viejo y burro).

---

## Correcta

ventana

## Error frecuente 1

bentana

## Error frecuente 2



## Regla

Ventana se escribe con v.

## Frecuencia

Muy frecuente

## Categoría

Casa

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (modismo con «ventana»); fitxes/lecturas del repo; contexto Aray ventana-ctx.

---

## Correcta

abuelo

## Error frecuente 1

avuelo

## Error frecuente 2



## Regla

Abuelo se escribe con b.

## Frecuencia

Muy frecuente

## Categoría

Casa

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo («Querido abuelo…»).

---

## Correcta

abuela

## Error frecuente 1

avuela

## Error frecuente 2



## Regla

Abuela se escribe con b.

## Frecuencia

Muy frecuente

## Categoría

Casa

## Imagen recomendable

Sí

## Observaciones

Vicens («Mi abuela se llama…»); lecturas del tomo 3.

---

## Correcta

biblioteca

## Error frecuente 1

viblioteca

## Error frecuente 2



## Regla

Biblioteca se escribe con b.

## Frecuencia

Muy frecuente

## Categoría

Colegio

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo; Savia; tomo 3 (sección biblioteca).

---

## Correcta

barco

## Error frecuente 1

varco

## Error frecuente 2



## Regla

Barco se escribe con b.

## Frecuencia

Frecuente

## Categoría

Objetos

## Imagen recomendable

Sí

## Observaciones

Vicens («viajar en barco»).

---

## Correcta

botella

## Error frecuente 1

votella

## Error frecuente 2



## Regla

Botella se escribe con b.

## Frecuencia

Frecuente

## Categoría

Objetos

## Imagen recomendable

Sí

## Observaciones

Vicens (utensilio para abrir botellas).

---

## Correcta

llave

## Error frecuente 1

llabe

## Error frecuente 2



## Regla

Llave se escribe con v.

## Frecuencia

Frecuente

## Categoría

Objetos

## Imagen recomendable

Sí

## Observaciones

Vicens (lista de palabras: pelota, llave…).

---

## Correcta

libro

## Error frecuente 1

livro

## Error frecuente 2



## Regla

Libro se escribe con b.

## Frecuencia

Muy frecuente

## Categoría

Colegio

## Imagen recomendable

Sí

## Observaciones

Vocabulario de biblioteca / lecturas escolares 3.º.

---

## Correcta

verano

## Error frecuente 1

berano

## Error frecuente 2



## Regla

Verano se escribe con v.

## Frecuencia

Muy frecuente

## Categoría

Naturaleza

## Imagen recomendable

Sí

## Observaciones

ANAYA / Savia / fábulas (verano e invierno).

---

## Correcta

invierno

## Error frecuente 1

inbierno

## Error frecuente 2



## Regla

Invierno se escribe con v.

## Frecuencia

Muy frecuente

## Categoría

Naturaleza

## Imagen recomendable

Sí

## Observaciones

Fábula hormiga/escarabajo (tomo 3).

---

## Correcta

verde

## Error frecuente 1

berde

## Error frecuente 2



## Regla

Verde se escribe con v.

## Frecuencia

Frecuente

## Categoría

Naturaleza

## Imagen recomendable

Sí

## Observaciones

ANAYA ampliación (poema «verde»); ANAYA refuerzo (falda verde).

---

## Correcta

cebada

## Error frecuente 1

cevada

## Error frecuente 2



## Regla

Cebada se escribe con b.

## Frecuencia

Frecuente

## Categoría

Naturaleza

## Imagen recomendable

Sí

## Observaciones

Fábula hormiga (trigo y cebada) en material 3.º.

---

## Correcta

nube

## Error frecuente 1

nuve

## Error frecuente 2



## Regla

Nube se escribe con b.

## Frecuencia

Frecuente

## Categoría

Naturaleza

## Imagen recomendable

Sí

## Observaciones

Vocabulario de lecturas / descripción de paisaje en ciclo medio; coherente con nivel 3.º. *(Respaldo menos explícito que verano/invierno: revisar en auditoría.)*

---

## Correcta

escribir

## Error frecuente 1

escrivir

## Error frecuente 2



## Regla

Los verbos terminados en -bir se escriben con b (escribir, recibir, subir…).

## Frecuencia

Muy frecuente

## Categoría

Acciones

## Imagen recomendable

No

## Observaciones

ANAYA ampliación (completar verbos en -bir: «Voy a ___ una carta…»).

---

## Correcta

recibir

## Error frecuente 1

recivir

## Error frecuente 2



## Regla

Los verbos terminados en -bir se escriben con b.

## Frecuencia

Muy frecuente

## Categoría

Acciones

## Imagen recomendable

No

## Observaciones

ANAYA ampliación («La actriz va a ___ un premio»).

---

## Correcta

subir

## Error frecuente 1

suvir

## Error frecuente 2



## Regla

Los verbos terminados en -bir se escriben con b.

## Frecuencia

Muy frecuente

## Categoría

Acciones

## Imagen recomendable

No

## Observaciones

ANAYA ampliación («vuelven a ___ los precios»).

---

## Correcta

vivir

## Error frecuente 1

bibir

## Error frecuente 2



## Regla

Vivir se escribe con v.

## Frecuencia

Muy frecuente

## Categoría

Acciones

## Imagen recomendable

No

## Observaciones

ANAYA refuerzo (definición «Vivir con alguien» junto a ejercicio b/v).

---

## Correcta

volver

## Error frecuente 1

bolver

## Error frecuente 2



## Regla

Volver se escribe con v.

## Frecuencia

Frecuente

## Categoría

Acciones

## Imagen recomendable

No

## Observaciones

ANAYA refuerzo (serie de verbos: volver, volar, valor…).

---

## Correcta

volar

## Error frecuente 1

bolar

## Error frecuente 2



## Regla

Volar se escribe con v.

## Frecuencia

Frecuente

## Categoría

Acciones

## Imagen recomendable

No

## Observaciones

Misma serie ANAYA refuerzo.

---

## Correcta

viaje

## Error frecuente 1

biaje

## Error frecuente 2



## Regla

Viaje se escribe con v.

## Frecuencia

Frecuente

## Categoría

Otros

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (entrada «viaje:»); ANAYA ampliación (texto del viaje).

---

## Correcta

valor

## Error frecuente 1

balor

## Error frecuente 2



## Regla

Valor se escribe con v.

## Frecuencia

Frecuente

## Categoría

Otros

## Imagen recomendable

No

## Observaciones

ANAYA refuerzo (serie volver / volar / valor).

---

## Correcta

pueblo

## Error frecuente 1

puevlo

## Error frecuente 2



## Regla

Pueblo se escribe con b.

## Frecuencia

Frecuente

## Categoría

Ciudad

## Imagen recomendable

Sí

## Observaciones

Fábula del burro (viaje al pueblo) en material 3.º.

---

## Correcta

viejo

## Error frecuente 1

biejo

## Error frecuente 2



## Regla

Viejo se escribe con v.

## Frecuencia

Frecuente

## Categoría

Otros

## Imagen recomendable

Sí

## Observaciones

Misma fábula (el niño, el viejo y el burro).

---

## Correcta

burbuja

## Error frecuente 1

vurbuja

## Error frecuente 2



## Regla

Burbuja se escribe con b.

## Frecuencia

Frecuente

## Categoría

Naturaleza

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (lista: mirador, transporte, verano, arboleda, burbuja).

---

## Correcta

padrino

## Error frecuente 1

padvino

## Error frecuente 2



## Regla

Padrino se escribe con b.

## Frecuencia

Poco frecuente

## Categoría

Casa

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (vocabulario: padrino / caballo). Prioridad menor.

---

## Correcta

veterinario

## Error frecuente 1

beterinario

## Error frecuente 2



## Regla

Veterinario se escribe con v.

## Frecuencia

Frecuente

## Categoría

Otros

## Imagen recomendable

Sí

## Observaciones

ANAYA ampliación («El veterinario lijó…»). Palabra larga pero aparece en ficha 3.º.

---

## Correcta

baraja

## Error frecuente 1

varaja

## Error frecuente 2



## Regla

Baraja se escribe con b.

## Frecuencia

Poco frecuente

## Categoría

Objetos

## Imagen recomendable

Sí

## Observaciones

ANAYA ampliación (poema de Lorca: baraja / bastos).

---

## Correcta

barro

## Error frecuente 1

varro

## Error frecuente 2



## Regla

Barro se escribe con b.

## Frecuencia

Frecuente

## Categoría

Naturaleza

## Imagen recomendable

Sí

## Observaciones

Mismo poema ANAYA («casa de barro»).

---

## Correcta

servicio

## Error frecuente 1

serbicio

## Error frecuente 2



## Regla

Servicio se escribe con v.

## Frecuencia

Frecuente

## Categoría

Otros

## Imagen recomendable

No

## Observaciones

ANAYA refuerzo (completar con b/v: ser_icio…).

---

## Correcta

recibo

## Error frecuente 1

recivo

## Error frecuente 2



## Regla

Recibo se escribe con b (familia de recibir / -bir).

## Frecuencia

Frecuente

## Categoría

Objetos

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (completar: reci_o).

---

## Correcta

subida

## Error frecuente 1

suvida

## Error frecuente 2



## Regla

Subida se escribe con b (familia de subir).

## Frecuencia

Frecuente

## Categoría

Otros

## Imagen recomendable

No

## Observaciones

ANAYA refuerzo (completar: su_ida).

---

## Correcta

vivienda

## Error frecuente 1

bibienda

## Error frecuente 2



## Regla

Vivienda se escribe con v.

## Frecuencia

Frecuente

## Categoría

Casa

## Imagen recomendable

Sí

## Observaciones

ANAYA refuerzo (completar: i_ienda).

---

## Correcta

haber

## Error frecuente 1

haver

## Error frecuente 2



## Regla

Haber se escribe con b (no con v). La h se trabaja en el banco H.

## Frecuencia

Frecuente

## Categoría

Acciones

## Imagen recomendable

No

## Observaciones

Vicens / ANAYA (formas de haber). Aquí solo el fallo b/v (*haver*), no *aber*.

---

## Resumen editorial

- **Número de registros (borrador 1):** 35
- **Fichas o carpetas revisadas:**
  - `verano_aray/banc_exercicis/_extraccio_raw/castellano_Lengua_3º-ANAYA_refuerzo.txt` (completar b/v; caballo; ventana; abuelo; biblioteca; verano; verde; viaje; volver/volar/valor; padrino; burbuja)
  - `verano_aray/banc_exercicis/_extraccio_raw/castellano_Lengua_3º-ANAYA_ampliacion.txt` (vaca; caballo; veterinario; verbos en -bir; baraja/barro; verde)
  - `verano_aray/banc_exercicis/_extraccio_raw/castellano_refuerzo_ampliacion_lengua_3.txt` (Vicens: caballos; botellas; barco; abuela; llave)
  - `verano_aray/banc_exercicis/_extraccio_raw/castellano_lenguaje3_tomo2_pdf.txt` (fábula burro/viejo/pueblo; verano/invierno/cebada; biblioteca; nota mb/nv → otro banco)
  - `verano_aray/banc_exercicis/_extraccio_raw/castellano_lengua_3_savia.txt` (biblioteca; verano)
  - `docs/TEMARIO_3_PRIMARIA_CATALUNYA.md` (b/v como regla de 3.º)
  - Contextos Aray `caballo-ctx` / `ventana-ctx` (alineación de nivel)
- **Bloques trabajados:**
  1. Vocabulario habitual con b o v (animales, casa, colegio, naturaleza…)
  2. Verbos en **-bir** (escribir, recibir, subir)
  3. Verbos / palabras de series ANAYA (vivir, volver, volar, valor, viaje)
  4. Ítems del ejercicio «completa con b o v» (servicio, recibo, subida, vivienda)
- **Descartes conscientes:**
  - *prohibición, exhibición, convivencia, concebir, percibir, hervidor…* del ejercicio ANAYA → demasiado difíciles o incompletos en el OCR
  - Conjugaciones (-aba, formas varias) → relleno / regla aparte
  - *tambor* y casos **mb/mp/nv** → `ERRORES_REALES_MPMB.md`
  - *beber, vaso, bicicleta…* → no ancladas con claridad en las extracciones
  - *blanco* → no hay error b/v infantil claro sin artificialidad
- **Confirmación:** borrador 1 sin generación libre de listas; errores = intercambio b↔v plausible.

### Estado editorial

- **Registros finales (borrador 1):** 35
- **Registros eliminados:** n/a (primer borrador)
- **Registros añadidos:** 35 (creación inicial)
- **Pendientes de revisar:** nube (respaldo más débil); veterinario / baraja / padrino (prioridad o longitud); haber (cruce con banco H)
- **Calidad editorial estimada:** 78 / 100 (falta auditoría + correcciones del ciclo)
- **Observaciones para la auditoría BV:**
  - Contrastar de nuevo el ejercicio ANAYA «completa con b o v» con el PDF original si el OCR deja dudas.
  - Decidir si *haber→haver* se queda aquí o solo en H.
  - No mezclar mb/mp.
  - Evitar ampliar con el bank técnico monolítico de lemmas.

### Dudas editoriales (para la auditoría)

1. **nube:** coherente con 3.º, pero menos explícita en fichas que verano/invierno.
2. **veterinario / baraja / padrino:** aparecen en fichas, pero prioridad o dificultad discutible.
3. **haber → haver:** ¿banco BV, solo H, o ambos con errores distintos?
4. **OCR del ejercicio b/v ANAYA:** algunas formas reconstruidas (servicio, recibo, subida, vivienda); conviene validar contra PDF.
5. Las fuentes del repo ejercitan poco b/v con listas cerradas (a diferencia de hie-/hue-); el borrador es necesariamente más corto que un bank inventado.
