...
describe('puntos de recompensa', () => {
  ...
  it('respeta tope diario con concesión parcial', () => {
    ...
    reward.dailyPoints = 80
    const grant = grantRewardPoints(
      reward,
      { requestedPoints: 50, sessionId: 's1', attemptIds: ['1'] },
      '2026-07-29',
    )
    expect(grant.granted).toBe(20)
  ...
  it('al llegar a 5000 marca premio y el sobrante pasa al siguiente ciclo', () => {
    ...
    const grant = grantRewardPoints(
      reward,
      { requestedPoints: 100, sessionId: 's2', attemptIds: [] },
      '2026-07-29',
    )
    ...
    expect(grant.reward.pointsTotal).toBe(50)
  ...
  it('con premio pendiente sigue concediendo al ciclo siguiente', () => {
    ...
    const grant = grantRewardPoints(
      reward,
      { requestedPoints: 50, sessionId: 's3', attemptIds: [] },
      '2026-07-29',
    )
    expect(grant.granted).toBe(50)
    expect(grant.reward.pointsTotal).toBe(150)
