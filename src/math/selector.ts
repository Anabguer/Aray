import { MIX_TABLES } from '@/config/playConfig'
 ...
 * Evita repetir la misma clave canónica de forma consecutiva.
 */
export function pickNextFact(
  ...
  const candidates = pool.filter((f) => factKeyOf(f) !== lastKey)
...
export function buildTrainQueue(
  ...
  for (let i = 0; i < count; i += 1) {
    const fact = pickNextFact(pool, progress, lastKey, random)
    lastKey = factKeyOf(fact)
    queue.push(toQuestionCard(fact, preferred, random))
  }
  return queue
}
...
export function buildMissesQueue(
  ...
  const pool: MultiplicationFact[] = missed.map(([key]) => {
  ...
  for (let i = 0; i < count; i += 1) {
    const fact = pickNextFact(pool, progress, lastKey, random)
    lastKey = factKeyOf(fact)
    queue.push(toQuestionCard(fact, null, random))
  }
  return { queue, usedFallbackMix: false }
}
