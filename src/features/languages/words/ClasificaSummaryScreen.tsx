...
const PLAY_PATH = '/missions/languages/words/clasifica'

export function ClasificaSummaryScreen() {
  ...
  return (
    <AppShell title="RESUMEN" shortTitle="Resumen" showBack backTo={WORDS_PATH}>
      <RoundSummary
        title={hot ? '¡Clasifica on fire!' : 'Buen repaso'}
        meta={state.title ?? 'Clasifica'}
        lumoState={hot ? 'celebration' : 'correct'}
        celebrate={hot}
        stats={[
          { value: correct, label: 'aciertos' },
          { value: total, label: 'palabras' },
